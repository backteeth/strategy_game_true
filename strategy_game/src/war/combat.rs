...
pub fn process_combat(military_registry: &mut MilitaryRegistry, war_registry: &WarRegistry) {
    // Group divisions by state
    let mut state_divisions: HashMap<StateId, Vec<crate::common::DivisionId>> = HashMap::new();
    ...
    for (_state_id, divisions) in state_divisions.into_iter() {
        if divisions.len() < 2 {
            continue;
        } // Need at least 2 divisions for combat

        // Very basic combat: check for hostilities
        // Find if any two divisions belong to countries at war
        let mut hostile_pairs = Vec::new();
        for i in 0..divisions.len() {
            for j in (i + 1)..divisions.len() {
                let a1 = military_registry.divisions.get(&divisions[i]).unwrap();
                let a2 = military_registry.divisions.get(&divisions[j]).unwrap();

                if war_registry.are_countries_at_war(a1.owner, a2.owner) {
                    hostile_pairs.push((divisions[i], divisions[j]));
                }
            }
        }
        ...
        for (id1, id2) in hostile_pairs {
            // Re-borrow divisions
            let a1 = military_registry.divisions.get(&id1).unwrap().clone();
            let a2 = military_registry.divisions.get(&id2).unwrap().clone();
            ...
            let a1_mut = military_registry.divisions.get_mut(&id1).unwrap();
            a1_mut.status = DivisionStatus::Fighting;
            ...
            let a2_mut = military_registry.divisions.get_mut(&id2).unwrap();
            a2_mut.status = DivisionStatus::Fighting;
